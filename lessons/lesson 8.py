...
#новое представление
def create_view_low_grades_chemic():
    cursor.execute('''
        CREATE VIEW IF NOT EXISTS grades AS
        SELECT fio, subject, grade
        JOIN grades ON user.userid = grades.userid
        WHERE grade <= 3
    ''')
    connect.commit()
    print('Представление grade создано')

create_view_low_grades_chemic()

def get_low_grades_chemic():
    cursor.execute("""
        SELECT * FROM low_grades_chemic
    """)
